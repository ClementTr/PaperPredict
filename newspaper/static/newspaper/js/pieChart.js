function donutChart() {
    var width,
        ...
        colour = d3v4.scaleOrdinal(d3v4.schemeCategory20c), // colour scheme
        ...
        selection.each(function(data) {
            ...
            var label = svg.select('.labelName').selectAll('text')
              ...
              .enter().append('text')
                .attr('dy', '.35em')
                .html(function(d) {
                    // console.log(d.data[category].length);
                    // console.log(d.data[category
                    // if(d.data[category].length > 20){
                    //     console.log(d.data[category].split(","))
                    // }else{
            ...
            var polyline = svg.select('.lines')
              ...
              .enter().append('polyline')
                .attr('points', function(d) {
                    ...
                    var pos = outerArc.centroid(d);
                    pos_array = [arc.centroid(d), outerArc.centroid(d), pos]
                    //pos[0] = radius * 0.95 * (midAngle(d) < Math.PI ? 1 : -1);
                    pos[0] = radius * 0.80 * (midAngle(d) < Math.PI ? 1 : -1);
                    if (pos[1] > -120) {
                      if (Math.abs(Math.max(pos_array[0], pos_array[2]) - Math.min(pos_array[0], pos_array[2])) > 10) {
                          return pos_array
                      }
                    }else{
                      return [0,0,0,0,0,0]
            ...
            function toolTipHTML(data) {
                ...
                for (var key in data.data) {
                    ...
                    if(key == "Value"){
                        ...
                        tip += '<tspan x="0" dy="0.4em">' + data.data["Segment"] + " " + value + '</tspan>';
                        // i++;
                    }
                }
